...
def get_spin_status(user) -> dict:
    ...
    # CLEAN: Only check DailySpinRecord
    ...
    return {
        ...
        'segments': [
            {'id': s.id, 'segment': s.order, 'order': s.order, 'label': s.label, 'reward_coins': s.reward_coins, 'weight': s.weight, 'color': s.color, 'is_active': s.is_active}
            for s in segments
        ]
    }
...
def process_daily_spin(user) -> dict:
    ...
    if DailySpinRecord.objects.filter(user=user, spin_date=today).exists():
        return {'already_spun': True, 'success': False, 'message': 'Come back tomorrow!'}
    ...
    with transaction.atomic():
        record = DailySpinRecord.objects.create(
            user=user, spin_date=today, segment_won=chosen_segment, coins_won=chosen_segment.reward_coins
        )
        
        # REMOVED legacy SpinWheelClaim.objects.get_or_create block
        ...
        WalletTransaction.objects.create(
            wallet=wallet, amount=coin_val, balance_after=wallet.balance, transaction_type='daily_spin',
            description="Daily Spin Wheel Win", reference_id=f"spin_{record.id}"
        )

        # Uses the canonical xp_engine instead of deleted xp_service
        xp_res = add_xp(user, 15, 'daily_spin')

        return {
            'success': True, 'already_spun': False, 'segment_won': {'id': chosen_segment.id, 'label': chosen_segment.label, 'reward_coins': chosen_segment.reward_coins, 'color': chosen_segment.color, 'order': chosen_segment.order, 'weight': chosen_segment.weight},
            'coins_won': chosen_segment.reward_coins, 'wallet_balance': float(wallet.balance),
            'message': f"Congratulations! You won {chosen_segment.reward_coins} Coins from the Daily Spin Wheel!",
            'segment_landed': chosen_segment.order, 'prize_type': 'coins', 'prize_value': str(chosen_segment.reward_coins), 'label': chosen_segment.label,
            'xp_earned': 15, 'level_info': xp_res,
        }
